...
export default function AddCart({ item }: { item: ItemsTypes }) {
        ...
        api.get(`/items/${item?.id}`)
            .then((res) => {
                ...
                dipatch(addCartItem({
                    ...
                    item_model: data?.models_name[0]?.colors[0]?.id,
                    maxCount: data?.models_name[0]?.colors[0]?.quantity,
                    price: data?.models_name[0]?.colors[0]?.price
                }))
            }).catch(err => {
                dipatch(setToastiState([{ type: 'error', data: err.message ?? 'Не удалось добавить в конзину!' }]))
    ...
    return (
        <div onClick={() => loading ? null : switchCart()} className={`cart-btn flex justify-center items-center text-primary  hover:text-white hover:bg-primary delay-500 ${isCart ? 'active' : ''}`}>
